...
- 扩展 `graph_search`：除现有 1 跳（直接 jurisdiction/topic/source），增加 2 跳"相关规则"——`(命中规则)-[:ABOUT]->(t:Topic)<-[:ABOUT]-(related:TaxRule)`，取同主题的其它规则作为 `related_rules`。**取前 N 用 `ORDER BY related.id LIMIT N` 保证确定性**（按主题重叠度/频次排序是后续优化，需真实 Neo4j 验证 count 聚合后再上）。
