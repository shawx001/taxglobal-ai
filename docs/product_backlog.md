...
| REQ-009 | 全收入合并计税,把 W-2/自雇/其它普通收入/资本利得/海外剩余等汇成一个总税 | 分块扩 `income_tax_summary` | 🟡 | 引擎三块已齐:Block 1(Step 2.7)覆盖 W-2 + 自雇 + 其它普通收入;Block 2(Step 2.8)覆盖资本利得和 NIIT;Block 3(Step 2.9)覆盖 FEIE 税率叠加和 NIIT MAGI 加回。FTC、州级 FEIE 差异和前端总览仍在后续。 |
