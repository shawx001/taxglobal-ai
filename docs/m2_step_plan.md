...
| Step | 标题 | 产出 | 依赖 | 状态 |
|---|---|---|---|---|
| M2.1 | 三库基础设施 | PostgreSQL + Neo4j + Chroma + embedding 模型 | — | ⬜ |
| M2.2 | 知识图谱建模 + 数据入库 | Neo4j schema + Chroma embeddings + 入库脚本 | M2.1 | ⬜ |
| M2.3 | GraphRAG 检索 API | `GET /api/knowledge/search`(图+向量混合) | M2.2 | ⬜ |
