...
| REQ-009 | 全收入合并计税,把 W-2/自雇/其它普通收入/资本利得/海外剩余等汇成一个总税 | 分块扩 `income_tax_summary` | 🟡 | Block 1(Step 2.7)已扩 W-2 + 自雇 + 其它普通收入:共享 Social Security wage base、合并 Additional Medicare threshold、QBI 排除 W-2,并保持 `w2_wages=0` 向后兼容。资本利得/NIIT、FEIE 税率叠加、FTC 和前端总览仍在后续 blocks。 |
...
- 合并 → `income_tax_summary` 已开始按 REQ-009 分块扩展:Step 2.7 覆盖 W-2 + 自雇 + 其它普通收入,后续再并入资本利得、FEIE/FTC 等。
