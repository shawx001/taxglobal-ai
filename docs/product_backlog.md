...
| REQ-003 | 删除前端遗留 caStateTax/nyStateTax，州税一律走后端 | 迁移自雇/W-2/6国对比模块到后端时 | 🟡 | Step 5.4 已让 `calcSE` 不再使用前端硬编码州税，改走 `/calc/income-summary`。`caStateTax/nyStateTax` 仍被 6 国对比/旧原型模块使用，本步保留；待这些模块迁移后再删除。 |
...
| REQ-012 | 加密资本利得接州税 | Step 2.6 后端数据+引擎；前端展示待下一步 | 🟡 | Step 2.6 给 `/calc/crypto` 增加可选 `state_code`:CA/NY/GA/IL/CO 按普通收入增量法计算州税,WA 按 2025 DOR capital gains excise(长期、$278,000 标准扣除、7%/9.9% 分档)计算。Step 5.5 前端仍只展示联邦横幅,后续需加州选择器和州税展示。 |
